...
class ConfigFileEventHandler(FileSystemEventHandler):
    ...
    async def put_new_item(self, item: typing.Any) -> None:
        await self.queue.put(item)
        return None

    async def get_new_item(self) -> typing.Any:
    ...
    def on_modified(self, event: DirModifiedEvent | FileModifiedEvent) -> None:
        if not event.is_directory:
            ...
            if Path(file_path).suffix != ".toml":
                return None
            future = asyncio.run_coroutine_threadsafe(self.put_new_item(item=file_path), loop=self.loop)
            try:
...
T = typing.TypeVar("T", str, Path)
"""
配置文件事件监控类 start
"""
class ConfigFileEventMonitor(object):
    def __init__(self, path: str | Path, loop: asyncio.AbstractEventLoop | None = None) -> None:
        self.loop = loop if loop else asyncio.get_running_loop()
        self.handler = ConfigFileEventHandler(loop=self.loop)
        self.observer = None
        self.path = self._check_path(path)
        self.monitoring_task = None

    @staticmethod
    def _check_path(path: T) -> T:
        _path = Path(path) if isinstance(path, str) else path
        if _path.exists():
            return path
        else:
            raise FileNotFoundError(f"找不到文件或目录{str(_path)}。")
    ...
    def start(self) -> None:
        if self.observer is None:
            ...
            self.observer.start()
            self.monitoring_task = self.loop.create_task(self.handler.update_settings())
        return None
